...
SELECT ROUND(AVG(salary), 2) as 평균임금, MAX(salary) 최고임금, MIN(salary) 최저임금, department_id as 부서번호
FROM employees
GROUP BY department_id
...
SELECT ROUND(AVG(salary), 0) as 평균임금, MAX(salary) 최고임금, MIN(salary) 최소임금, job_id as 업무아이디
FROM employees
GROUP BY job_id
...
-- 문제 10.
-- 보너스 지금을 위해 입사일 기준으로 나누기
-- 02/12/31 이전 입사자는 '창립멤버'
-- 03냔 입사자 '03년입사', 04년 입사자 '04년입사', 이후 입자자 '상장이후입사'
-- optDate 컬럼의 데이터로 출력
-- 정렬은 입사일로 오름차순
